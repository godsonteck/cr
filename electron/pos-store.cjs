...
function createPosStore(userDataPath) {
  const db = new DatabaseSync(path.join(userDataPath, 'cr-pos.sqlite'));
  ...
  return {
    ...
    cacheCatalog(products) {
      ...
      try { products.forEach(product => upsert.run(product.id, JSON.stringify(product), now)); db.exec('COMMIT'); }
