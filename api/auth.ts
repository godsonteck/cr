...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (method === 'POST') {
      ...
      if (action === 'korapay-initialize') {
        const auth = await requireAuth(req, res);
        if (!auth) return;
        const parsed = korapayInitializeSchema.safeParse(body);
        const secretKey = process.env.KORAPAY_SECRET_KEY?.trim();
        if (!parsed.success || !secretKey) return res.status(500).json({ error: 'Korapay is not configured on the server' });
