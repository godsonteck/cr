...
Please confirm availability and dispatch to Accra. Thank you!`;
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/65 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
      <div 
        className="bg-white rounded-3xl max-w-3xl w-full p-6 sm:p-8 shadow-2xl border border-rose-100 relative space-y-6 max-h-[90vh] overflow-y-auto"
        onClick={e => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-full text-gray-400 hover:text-gray-900 hover:bg-gray-100 transition-colors z-10 cursor-pointer"
          aria-label="Close"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 items-start">
          ...
          <div className="space-y-3">
            <div className="w-full aspect-square rounded-2xl bg-[#FAF6F4] p-4 flex items-center justify-center border border-gray-100 relative">
              <img
                src={product.image}
                alt={product.name}
              ...
              />
              {product.badge && (
                <span className="absolute top-3 left-3 bg-[#8A3D52] text-white text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-2xs">
                  {product.badge}
                </span>
              )}
            </div>
            ...
            {/* Authenticity Badge */}
            <div className="bg-rose-50/70 border border-rose-100 rounded-xl p-3 text-xs text-[#8A3D52] flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 shrink-0 text-[#8A3D52]" />
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <h2 className="text-lg sm:text-xl font-serif font-bold text-gray-900 mt-0.5">
                {product.name}
              </h2>
            ...
            >
              <div className="flex text-[#D4AF37]">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star 
                    key={star} 
                    className={`w-3.5 h-3.5 ${star <= Math.round(averageRating) ? 'fill-current text-[#D4AF37]' : 'text-gray-300'}`} 
                  />
                ))}
              </div>
              <span className="font-bold text-gray-800">{averageRating.toFixed(1)}</span>
              <span className="text-gray-400 underline decoration-rose-200 group-hover:text-[#8A3D52]">
            ...
            <div className="space-y-1.5 pt-1">
              {product.highlights.map((h, i) => (
                <div key={i} className="flex items-center gap-2 text-xs text-gray-700">
                  <Check className="w-3.5 h-3.5 text-[#8A3D52] shrink-0" />
                  <span>{h}</span>
                </div>
              ))}
            </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-gray-200 rounded-lg bg-gray-50 p-1">
                  <button
                    ...
                    className="p-1.5 text-gray-600 hover:text-black rounded hover:bg-white transition-colors cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="p-2.5 rounded-lg border border-gray-200 hover:bg-rose-50 text-gray-400 hover:text-[#8A3D52] transition-colors cursor-pointer"
                  aria-label="Wishlist"
                >
                  <Heart className={`w-4 h-4 ${isFavorited ? 'fill-[#8A3D52] text-[#8A3D52]' : ''}`} />
                </button>
              </div>
        ...
        <div className="pt-4 border-t border-gray-100 text-xs">
          ...
          <div className="flex flex-wrap gap-4 border-b border-gray-100 pb-2 font-bold text-gray-600">
            <button
              onClick={() => setActiveTab('benefits')}
              className={`pb-1 transition-colors cursor-pointer ${activeTab === 'benefits' ? 'text-[#8A3D52] border-b-2 border-[#8A3D52]' : 'hover:text-gray-900'}`}
            >
              Key Benefits
            </button>
            {product.details?.howToUse && (
              <button
                ...
                className={`pb-1 transition-colors cursor-pointer ${activeTab === 'how-to-use' ? 'text-[#8A3D52] border-b-2 border-[#8A3D52]' : 'hover:text-gray-900'}`}
              >
                How To Use
              </button>
            )}
            {product.details?.ingredients && (
              <button
                ...
                className={`pb-1 transition-colors cursor-pointer ${activeTab === 'ingredients' ? 'text-[#8A3D52] border-b-2 border-[#8A3D52]' : 'hover:text-gray-900'}`}
              >
                Ingredients
              </button>
            )}
            <button
              onClick={() => setActiveTab('reviews')}
              className={`pb-1 transition-colors cursor-pointer flex items-center gap-1.5 ${activeTab === 'reviews' ? 'text-[#8A3D52] border-b-2 border-[#8A3D52]' : 'hover:text-gray-900'}`}
            >
          ...
          <div className="pt-3 text-gray-600 leading-relaxed">
            ...
            {activeTab === 'ingredients' && (
              <p className="font-mono text-[11px] bg-gray-50 p-3 rounded-xl border border-gray-100">{product.details?.ingredients}</p>
            )}
            ...
            {activeTab === 'reviews' && (
              <div className="space-y-6 pt-1">
                ...
                <div className="bg-[#FAF5F4] p-4 sm:p-5 rounded-2xl border border-rose-100/90 flex flex-col md:flex-row items-center justify-between gap-6">
                  
                  {/* Left score summary */}
                  <div className="flex items-center gap-4 text-center sm:text-left">
                    <div>
                      <span className="text-3xl sm:text-4xl font-serif font-extrabold text-gray-900 block">
                      ...
                      <div className="flex text-[#D4AF37] justify-center sm:justify-start mt-0.5">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star 
                            key={star} 
                            className={`w-3.5 h-3.5 ${star <= Math.round(averageRating) ? 'fill-current' : 'text-gray-300'}`} 
                          />
                        ))}
                      </div>
                      ...
                      {[5, 4, 3, 2, 1].map((ratingNum) => {
                        ...
                        return (
                          <div key={ratingNum} className="flex items-center gap-2 text-[10px]">
                            <span className="w-3 font-bold text-gray-500">{ratingNum}★</span>
                            <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-[#8A3D52] rounded-full transition-all"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                ...
                {isWritingReview && (
                  <form onSubmit={handleReviewSubmit} className="bg-white border-2 border-[#8A3D52]/30 rounded-2xl p-5 space-y-4 animate-fadeIn shadow-sm">
                    <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                      <h4 className="font-serif font-bold text-sm text-gray-900 flex items-center gap-2">
                        <Award className="w-4 h-4 text-[#8A3D52]" />
                        <span>Write Your Verified Review</span>
                      </h4>
                      <span className="text-[11px] text-[#8A3D52] font-semibold">
                    ...
                    {/* Star selector */}
                    <div>
                      <label className="block text-[11px] font-bold text-gray-700 mb-1">Your Rating *</label>
                      <div className="flex items-center gap-1.5">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <button
                            ...
                            className="p-1 text-gray-300 hover:text-[#D4AF37] transition-colors cursor-pointer"
                          >
                            <Star className={`w-6 h-6 ${(hoverRating || reviewRating) >= star ? 'fill-[#D4AF37] text-[#D4AF37]' : 'text-gray-300'}`} />
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      <div>
                        <label className="block text-[11px] font-bold text-gray-700 mb-1">Your Name / Alias *</label>
                        <input
                          type="text"
                          required
                          value={reviewerName}
                          ...
                          className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#8A3D52]"
                        />
                      </div>

                      <div>
                        <label className="block text-[11px] font-bold text-gray-700 mb-1">Your Skin Profile / Interest</label>
                        <select
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-[11px] font-bold text-gray-700 mb-1">Headline / Review Title</label>
                        <input
                          type="text"
                          value={reviewTitle}
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-[11px] font-bold text-gray-700 mb-1">Detailed Review & Texture Experience *</label>
                        <textarea
                          required
                          rows={3}
                          value={reviewComment}
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2">
                      <button
                        ...
                        className="px-4 py-2 border border-gray-200 text-gray-600 rounded-lg text-xs font-bold hover:bg-gray-50 cursor-pointer"
                      >
                        Cancel
                      </button>
                      <button
                ...
                <div className="space-y-4">
                  {productReviews.length === 0 ? (
                    <div className="text-center py-6 text-gray-400 space-y-1">
                      <p className="font-semibold text-gray-600">Be the first to review this original item!</p>
                      ...
                      <div key={rev.id} className="p-4 bg-gray-50/70 rounded-2xl border border-gray-100 space-y-2">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex text-[#D4AF37]">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <Star 
                                  key={star} 
                                  className={`w-3 h-3 ${star <= rev.rating ? 'fill-current' : 'text-gray-300'}`} 
                                />
                              ))}
                            </div>
                            <span className="font-bold text-gray-900 text-xs">{rev.authorName}</span>
                            {rev.verifiedPurchase && (
                              <span className="bg-emerald-50 text-emerald-700 text-[10px] font-bold px-2 py-0.2 rounded-full flex items-center gap-0.5 border border-emerald-200">
                        ...
                        {rev.skinType && (
                          <div className="inline-block">
                            <span className="text-[10px] bg-rose-50 text-[#8A3D52] font-semibold px-2 py-0.5 rounded-md border border-rose-100">
                              Skin: {rev.skinType}
                            </span>
                          </div>
                        )}
                        ...
                        {/* Title & Comment */}
                        {rev.title && (
                          <h5 className="font-bold text-gray-900 text-xs">{rev.title}</h5>
                        )}
                        ...
                        <div className="flex items-center justify-between pt-1 text-[11px] text-gray-400">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 hover:text-gray-700 cursor-pointer transition-colors"
                          >
