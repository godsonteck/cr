...
export function AdminPOSScreen() {
  ...
  const shownProducts = useMemo(() => {
    ...
    return !needle ? catalogueItems : catalogueItems.filter(item => [item.title, item.product.brand, item.product.category, item.product.id, item.variant?.id, item.barcode].filter(Boolean).join(' ').toLowerCase().includes(needle));
